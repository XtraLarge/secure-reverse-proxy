...
# The geo access log (client IP + country) is written to a file for local
# forensics (CustomLog .../geo_access.log) AND mirrored to syslog:local5 by the
# LOGGING macro. When a remote syslog target is configured, forward the local5
# stream to the central collector so client-IP forensics survive container
# rebuilds and is queryable centrally. Kept on its own facility (local5) so the
# collector can label the access stream independently from the local7 error/
# consolidated stream. No file action here — the file copy is Apache-native.
...
if [[ -n "${SYSLOG_REMOTE}" ]]; then
  ...
  log "rsyslog: geo access-log forwarding (local5) → ${_proto}://${_rhost}:${_rport}"
else
  rm -f "$GEO_ROUTING"
fi
