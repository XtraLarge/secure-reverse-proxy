...
# ── Geo access-log forwarding ────────────────────────────────────────────
# The geo access log (client IP + GeoIP country) is written to a file for local
# forensics (CustomLog .../geo_access.log, per-vhost). To make that forensic
# stream queryable centrally and survive container rebuilds, rsyslog tails the
# SAME file via imfile (out of Apache's request path) and forwards it to the
# collector. imfile is used instead of a piped CustomLog because Apache spawns
# one logger(1) process PER vhost CustomLog directive (no dedup) — with many
# vhosts that is hundreds of processes. imfile is a single reader.
# Tagged apache_geo on facility local5 so the collector labels it independently
# from the local7 error/consolidated stream.
GEO_ROUTING="/etc/rsyslog.d/21-geo-forward.conf"
if [[ -n "${SYSLOG_REMOTE}" ]]; then
  {
    echo "# Generated by entrypoint.sh — do not edit"
    echo "module(load=\"imfile\")"
    echo "input(type=\"imfile\""
    echo "      File=\"/var/log/apache2/geo_access.log\""
    echo "      Tag=\"apache_geo\""
    echo "      Facility=\"local5\""
    echo "      Severity=\"info\""
    echo "      reopenOnTruncate=\"on\")"
    echo "if \$syslogfacility-text == 'local5' then {"
    echo "    action(type=\"omfwd\" target=\"${_rhost}\" port=\"${_rport}\" protocol=\"${_proto}\" Template=\"RSYSLOG_SyslogProtocol23Format\")"
    echo "    stop"
    echo "}"
  } > "$GEO_ROUTING"
  log "rsyslog: geo access-log forwarding (imfile→local5) → ${_proto}://${_rhost}:${_rport}"
else
  rm -f "$GEO_ROUTING"
fi
